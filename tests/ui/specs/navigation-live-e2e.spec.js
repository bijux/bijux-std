...
test.describe("live navigation release gate", () => {
  ...
  test("hub phone drawer renders local sections before sites", async ({ page }, testInfo) => {
    ...
    const sections = page.locator('[data-bijux-mobile-order="1-sections"]');
    ...
    await expectLocalBeforeSites(page, '[data-bijux-mobile-order="1-sections"]');
  });
  ...
  test("hub phone Home exposes overview pages without top-level leakage", async ({ page }, testInfo) => {
    ...
    const sectionsText = await page.locator('[data-bijux-mobile-order="1-sections"]').innerText();
  ...
  test("project root phone drawer keeps local tree before global sites", async ({ page }, testInfo) => {
    ...
    const rowOne = page.locator('[data-bijux-mobile-order="1-top-directories"]');
    await expect(rowOne).toBeVisible();
    await expect(rowOne.locator('.md-nav__link').first()).toBeVisible();
    await expectLocalBeforeSites(page, '[data-bijux-mobile-order="1-top-directories"]');
  });
  ...
  test("project phone row model progresses into deeper rows on at least one branch", async ({ page }, testInfo) => {
    ...
    const rowLinks = page.locator('[data-bijux-mobile-order="1-top-directories"] .md-nav__link');
    ...
    for (const href of candidates.slice(0, 4)) {
      ...
      if (await page.locator('[data-bijux-mobile-order="2-subdirectories"]').isVisible()) {
    ...
    await expect(page.locator('[data-bijux-mobile-order="2-subdirectories"]')).toBeVisible();
    await expectLocalBeforeSites(page, '[data-bijux-mobile-order="2-subdirectories"]');
  });
  ...
  test("phone drawer avoids duplicate wrapper entries on hub and project roots", async ({ page }, testInfo) => {
    ...
    const hubTexts = await page
      .locator('[data-bijux-mobile-order="1-sections"] .md-nav__link')
    ...
    const projectTexts = await page
      .locator('[data-bijux-mobile-order="1-top-directories"] .md-nav__link')
      .evaluateAll((nodes) => nodes.map((node) => (node.textContent || "").trim()).filter(Boolean));
